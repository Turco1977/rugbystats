...
import { EntrenadorFixture } from "@/components/entrenador/entrenador-fixture";
...
type EntrenadorTab = "partidos" | "fixture";

const APP_URL = typeof window !== "undefined" ? window.location.origin : "https://rugbystats-five.vercel.app";

/** Derive division from user email: m19@lostordos.com → M19 */
function divisionFromEmail(email: string): string {
  const match = email.match(/^(m\d+)@/i);
  return match ? match[1].toUpperCase() : "M19";
}
...
function EntrenadorPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntrenadorTab>("partidos");

  // Division derived from email, with URL param as fallback
  const division = userEmail ? divisionFromEmail(userEmail) : divParam || "M19";
  ...
  return (
    <main className="min-h-screen bg-dk-1 text-white">
        ...
        <div className="flex max-w-3xl mx-auto">
          <button
            ...
            className={`flex-1 text-center py-3 text-sm font-bold transition-colors ${
              activeTab === "partidos"
                ? "text-white border-b-2 border-gn"
                : "text-dk-4 hover:text-white"
            }`}
          >
            Partidos
          </button>
      ...
      <div className="p-4 max-w-3xl mx-auto">
        {activeTab === "fixture" ? (
          <EntrenadorFixture division={division} />
        ) : (
          <>
            {/* Summary */}
