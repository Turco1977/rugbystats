...
export default function HistorialPage() {
  ...
  // Detect available ramas for current division
  ...
  useEffect(() => {
    async function fetchHistorial() {
      ...
      // Fetch partidos - filter by division, optionally by rama
      let query = supabase
        .from("partidos")
        .select(`
      ...
      let matchData = (data || []) as unknown as PartidoHistorial[];

      // If rama filter returned 0 but "Todos" has data, rama might not be set
      // Fallback: show all for division
      if (matchData.length === 0 && selectedRama !== "Todos") {
        ...
        matchData = (allData || []) as unknown as PartidoHistorial[];
      }

      setPartidos(matchData);

      // Detect available ramas
      const ramas = [...new Set(matchData.map((p) => p.rama).filter(Boolean))];
      setAvailableRamas(ramas);

      // Fetch ALL eventos for these partidos (including data for drill-downs)
      if (matchData.length > 0) {
        ...
        setEventos((evData || []) as unknown as EventoHistorial[]);
      } else {
        setEventos([]);
      }
  ...
  const draws = partidos.filter((p) => p.puntos_local === p.puntos_visitante).length;
  ...
  if (selectedModule) {
    ...
    const rivalEvents = hasPerspective
      ? moduleEvents.filter((ev) => ev.perspectiva === "rival")
      : [];
    ...
    const motivoDrillEvents = motivoDrill
      ? propioEvents.filter((ev) => ev.data?.motivo === motivoDrill)
      : [];
    const motivoDrillResultados: Record<string, number> = {};
    motivoDrillEvents.forEach((ev) => {
      const res = ev.data?.resultado || "?";
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-g-4">
              Acumulado {selectedDivision}{selectedRama !== "Todos" ? ` ${selectedRama}` : ""} — {partidos.length} partidos
            </p>
          </div>
        </div>
    ...
    <div>
      ...
      <div className="flex gap-2 mb-3 overflow-x-auto pb-1">
        {DIVISIONS.map((div) => (
          <button
            key={div}
            onClick={() => { setSelectedDivision(div); setSelectedRama("Todos"); setSelectedModule(null); }}
      ...
      <div className="flex gap-2 mb-4">
        <button
          ...
          className={`text-[10px] font-bold px-3 py-1.5 rounded transition-colors ${
            "Todos" === selectedRama ? "bg-gn text-white" : "bg-dk-2 border border-dk-3 text-g-4 hover:bg-dk-3"
          }`}
        >
          Todos
        </button>
        {(availableRamas.length > 0 ? availableRamas : ["A", "B", "C"]).map((rama) => (
          <button
            ...
            className={`text-[10px] font-bold px-3 py-1.5 rounded transition-colors ${
              rama === selectedRama ? "bg-gn text-white" : "bg-dk-2 border border-dk-3 text-g-4 hover:bg-dk-3"
            }`}
          >
      ...
      <div className="grid grid-cols-5 gap-2 mb-6">
        <div className="card-compact text-center">
          ...
          <p className="text-xl font-extrabold text-nv dark:text-white">{partidos.length}</p>
        </div>
        <div className="card-compact text-center">
                ...
                return (
                  <tr key={p.id} className="border-t border-dk-3 hover:bg-dk-2 transition-colors">
                    ...
                    <td className="py-2.5 px-3 text-right">
                      <a href={`/partido/${p.id}`} className="text-[10px] text-bl hover:underline">Ver →</a>
                    </td>
                  </tr>
                );
              })
      ...
      <h3 className="text-[10px] font-bold text-g-4 uppercase tracking-wider mb-3">
        Acumulado por Módulo — Tocá para ver detalle
      </h3>
              ...
              {total > 0 ? (
                <>
                  <div className="text-center mb-2">
                    <span className="text-lg font-extrabold text-nv dark:text-white">{total}</span>
                  </div>
      ...
      {/* Eficacia en 22 acumulada */}
      <div className="mb-6">
        <Eficacia22Card eventos={eventos} />
      </div>
      ...
      {(() => {
        ...
        const ICON_MAP: Record<string, string> = {
          tarjeta_roja: "🟥", tarjeta_amarilla: "🟨", lesion: "🏥", publico: "👥", disciplina: "⚠️",
        };
                  ...
                  return (
                    <div key={ev.id} className="card-compact flex items-center gap-2">
                      ...
                      <div className="flex-1">
                        <span className="text-xs font-bold text-nv capitalize">{tipo.replace(/_/g, " ")}</span>
                        {nombre && <span className="text-xs text-g-5"> — {nombre}</span>}
                        {desc && <p className="text-[10px] text-g-4">{desc}</p>}
                      </div>
                      {ev.tiempo && (
...
function Comparador({ division }: { division: string }) {
  ...
  const handleCompare = useCallback(async () => {
    ...
    setStatsA(resultA.stats); setStatsB(resultB.stats);
    setSummaryA(resultA.summary); setSummaryB(resultB.summary);
  ...
  return (
    <div className="mt-2">
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <select value={ramaA} onChange={(e) => setRamaA(e.target.value)} className="text-xs px-3 py-2 rounded-md bg-dk-2 border border-dk-3 text-white font-bold">
        ...
        <select value={ramaB} onChange={(e) => setRamaB(e.target.value)} className="text-xs px-3 py-2 rounded-md bg-dk-2 border border-dk-3 text-white font-bold">
          <option value="A">{division} A</option><option value="B">{division} B</option><option value="C">{division} C</option>
        </select>
        <button onClick={handleCompare} disabled={ramaA === ramaB || loadingCompare} className="text-xs font-bold px-5 py-2 rounded-md bg-gn text-white hover:bg-gn-dark disabled:opacity-40 transition-colors">
              ...
              return (
                <div key={mod.id} className="card-compact">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    {better !== "=" && (
                      <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${better === "A" ? "bg-nv/20 text-nv dark:text-white" : "bg-bl/20 text-bl"}`}>
